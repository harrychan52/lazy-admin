<template>
  <Form :model="formData" ref="loginFormRef" :rules="loginRules" size="large" @finish="login">
  ...
  <div class="login-btn">
    <Button type="primary" size="large" @click="resetForm">重置</Button>
    <Button type="primary" html-type="submit" size="large" :loading="loading"> 登录 </Button>
  </div>
</template>
<script lang="ts" setup>
  ...
  // login
  const login = async () => {
    const result = loginFormRef.value.validateFields();
    console.log(result);
