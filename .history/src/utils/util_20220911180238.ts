...
export function handleRouter(routerList: Menu[], newArr: string[] = [], parentPath?: string) {
  routerList.forEach((item: Menu) => {
    ...
    const  prefixPath = parentPath?newArr.push(item.path):newArr.push(parentPath + '/' + item.path);
   
   
    if (item.children) 
      handleRouter(item.children, newArr, prefixPath);
    }
    const arr: string[] = [];
    item.path && newArr.push(item.path);
    if (item.children) {
      handleRouter(item.children, arr, item.path);
      newArr.push(...arr);
    }
  });
  return newArr;
