 ...
 * @return array
 */
export function handleRouter(routerList: Menu[], newArr: string[] = [], parentPath?: string) {
  const getRoutes = (parentPath: string, routerList: Menu[], arr: string[] = []) => {
    routerList.forEach((item) => {
      const prefixPath = parentPath + '/' + item.path;
      if (!item.children) {
        return arr.push(prefixPath);
      } else {
        return getRoutes(prefixPath, item.children, arr);
      }
    });
  };
  routerList.forEach((item: Menu) => {
    ...
    const arr: string[] = [];
    parentPath && newArr.push(parentPath);
    if (item.children) {
      getRoutes(parentPath, item.children, arr);
      newArr.push(...arr);
    }
  });
  return newArr;
