 ...
 * @return array
 */
export function handleRouter(routerList: Menu[], newArr: string[] = []) {
  function getRoutes (parentPath: string,routerList: Menu[], arr: string[] = []):string[]{
    routerList.forEach(item=>{
      if(!item.children){
        return  arr.push('/'+parentPath+item.path)
      }
      else{
        return getRoutes(item.path,item.children,arr)
      }
    })
  }
  routerList.forEach((item: Menu) => {
    const parentPath = item.path
    const arr:string[] = []
    if(!item.children){
      return newArr.push('/'+parentPath)
    }else{
      const childrenArr:string[] = getRoutes(parentPath,item.children,arr)
      return newArr.concat(childrenArr)
    }
